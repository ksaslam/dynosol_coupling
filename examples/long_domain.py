...
# input parameters
out_file= 'long_domain.poly'
height_of_domain= 30     #(km)
height_pert = 2
len_of_fault=40
number_of_point_along_fault= 801


#x_rough= np.loadtxt("rough_x.out")

#y_rough= np.loadtxt("rough_y.out")
#x_rough= np.linspace(0,len_of_fault, len(y_rough) )

x_rough = np.linspace( 0, len_of_fault, number_of_point_along_fault )

len_arrays= 2*len(x_rough) + 4   #(4 corners + 4 points on the edge of the fault zone )

fault_loc_y= 30 *1000/2.


y_rough = seistools.rough.generate_profile(number_of_point_along_fault, len_of_fault, 0.01, 20, 1, 1000)
y_rough= y_rough*1000 + fault_loc_y 
np.savetxt( 'x.out', x_rough)
np.savetxt ('y.out', y_rough)

plt.plot(x_rough, y_rough)
plt.show()
...
zones_2_x = 0.0
zones_2_y = fault_loc_y + 100.
#zones_2_y=  10100.0
element_size_2 = 10000.0
element_type_2= 1

zones_3_x = 0.0
zones_3_y=  height_of_domain *1000 - 100	
#zones_3_y=  15000.0
